...
import 'package:dotube/configs/routes.dart';
import 'package:dotube/components/shared/dialogs/prompt_dialog.dart';
...
class AuthenticationCredentials {
  ...
  static Future<AuthenticationCredentials> fromCookie(String cookie) async {
    try {
      final spDc =
          cookie.split("; ").firstWhereOrNull((c) => c.trim().startsWith("sp_dc="))?.trim();
      final res = await get(
        Uri.parse(
      ...
      if (rootNavigatorKey?.currentContext != null) {
        showPromptDialog(
          ...
          title: rootNavigatorKey!.currentContext!.l10n.error("Authentication Failure"),
          message: e.toString(),
          cancelText: null,
        );
      }
...
class AuthenticationNotifier extends PersistedStateNotifier<AuthenticationCredentials?> {
