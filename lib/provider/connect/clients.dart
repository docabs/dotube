...
import 'package:dotube/services/device_info/device_info.dart';
...
class ConnectClientsNotifier extends AsyncNotifier<ConnectClientsState> {
  ...
  @override
  build() async {
    final discovery = BonsoirDiscovery(type: '_dotube._tcp');
    ...
    final subscription = discovery.eventStream?.listen((event) {
      ...
      switch (event.type) {
        ...
        case BonsoirDiscoveryEventType.discoveryServiceLost:
          state = AsyncData(
            ConnectClientsState(
              services: state.value!.services.where((s) => s.name != event.service!.name).toList(),
...
final connectClientsProvider = AsyncNotifierProvider<ConnectClientsNotifier, ConnectClientsState>(
  () => ConnectClientsNotifier(),
);
