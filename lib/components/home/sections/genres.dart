...
import 'package:dotube/provider/spotify/spotify.dart';

import 'package:dotube/configs/fake.dart';
              ...
              children: [
                ...
                Directionality(
                  ...
                  child: TextButton.icon(
                    ...
                    icon: const Icon(DotubeIcons.angleRight),
                    label: Text(
          ...
          sliver: Skeletonizer.sliver(
            ...
            child: SliverGrid.builder(
              ...
              itemBuilder: (context, index) {
                final category = categories.elementAtOrNull(index) ?? FakeData.category;

                return HookBuilder(builder: (context) {
                  final (:gradient, :textColor) = useMemoized(
                    () {
                      final gradient = gradients[Random().nextInt(gradients.length)];
                      final text = gradient.colors.take(2).any((c) => c.computeLuminance() > 0.5)
                          ? Colors.grey[900]
                          : Colors.white;
                      return (
                        gradient: LinearGradient(
                          colors: gradient.colors.map((c) => c.withOpacity(0.8)).toList(),
                        ),
                        textColor: text
                      );
                    },
